...
- Image editing is handled by `Infrastructure/PreviewImageEditor.swift`, which launches Preview.app as an external process with a pre-prepared `~/Downloads/.ClipboardManagerEdit.<ext>` working file.
...
- Session completion is detected by AX window destruction, AX window-existence polling, or `NSWorkspace.didTerminateApplicationNotification`. A 5-minute idle timeout stops monitoring and preserves the working file rather than treating the edit as complete.
- Accessibility permission is recommended for instant window-close detection but is not required; without it the app falls back to Preview app termination.
...
- Never automatically delete or overwrite a preserved `.ClipboardManagerEdit.<ext>` file; it may be the only recoverable copy after a failed save or interrupted shutdown.
